...
import { createMateriaPrima } from "@/app/actions/materia-prima";
import { fetchProveedores } from "@/app/actions/proveedor";
import { fetchProductos } from "@/app/actions/productos";
import type { MateriaPrimaFormData } from "@/types/materia-prima";
...
export function MateriaPrimaForm({ onSuccess }: Props) {
  ...
  const [formData, setFormData] = useState<MateriaPrimaFormData>({
    ...
    codigo: "",
    descripcion: "",
    cantidad_revisada: null as any,
    cantidad_aceptada: null as any,
    cantidad_rechazada: null as any,
    referencia: "Referencia al Plan de Calidad",
    comentario: "",
    certificado: "no es necesario",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createMateriaPrima(formData);
      if (result.error) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-black">Nuevo Registro de Materia Prima</h2>
              <button
